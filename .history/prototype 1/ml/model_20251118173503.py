...
from cost_calculator import cost_calculator  # Import the instance directly
...
class ETAModel:
    ...
    def predict_with_cost(self, start_coords, end_coords, hour, day, 
        ...
        # Calculate distance
        distance_km = cost_calculator.calculate_distance(start_coords, end_coords)
        ...
        if self.trained:
            ...
            time_estimate = cost_calculator.estimate_travel_time(
                distance_km, time_of_day
            )
            travel_time_minutes = time_estimate['travel_time_minutes']
        
        # Calculate comprehensive cost analysis
        cost_analysis = cost_calculator.calculate_driving_cost(
    ...
    def compare_vehicles_with_ml(self, start_coords, end_coords, hour, day):
        ...
        if self.trained:
            ...
            time_estimate = cost_calculator.estimate_travel_time(distance_km, time_of_day)
            ml_travel_time = time_estimate['travel_time_minutes']
        
        # Get vehicle comparisons
        vehicle_comparison = cost_calculator.compare_vehicles(
            start_coords, end_coords, time_of_day
        )
        
        # Enhance comparison with ML predictions
        for vehicle in vehicle_comparison['all_vehicles']:
            vehicle['ml_predicted_travel_time'] = round(ml_travel_time, 2)
        
        vehicle_comparison['ml_predicted_travel_time'] = round(ml_travel_time, 2)
        vehicle_comparison['distance_km'] = round(distance_km, 2)
